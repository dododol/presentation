		...
		<div id="content">
			...
			<div class="navigation_help_depth3">
				<ul>
					...
					<li><a class="selected"
						href="<%=request.getContextPath()%>/member/qna/qnaList"
						data-nav-track="ga_lead|section-gnb|help|notice_event"><span>1:1문의</span></a></li>
					<li><a class=""
						href="<%=request.getContextPath()%>/eventIng.do"
			...
			<div class="inquiryViewWrap">
				...
				<div class="inquiryView">
					<div class="viewCont">
						<dl class="inqCont">
							<dt>
								<strong>[문의]</strong> ${qna.qstnTitle } <q>${qna.qstnDate }</q>
							</dt>
					...
					<div class="viewCont">
						<div class="reply">
							<c:if test="${sessionScope.authId eq 'a001'}"> <!-- 로그인 한 아이디가 a001(관리자)이면 댓글등록창이 보인다. -->
								<c:if test="${empty reply }">
									<form id="frm" name="frm" method="post"
										action="/FinalProject/member/createPost">
										<p>댓글을 입력하세요.</p>
										<input type="hidden" name="qstnNo" value="${qstnNo}" />
										<textarea rows="3" cols="10" id="rpyContent" name="repContent"
											style="display: none;"></textarea>
										<textarea rows="" cols="" id="textareaText"
											name="textareaText"></textarea>
							...
							<br> <br> <br>
							<c:if test="${not empty reply }"> <!-- reply가 비어있지 않으면 답변내용이 보여진다. -->
								<div id="replyContent">
									<dl class="inqCont">
										<dt>
											<strong class="replyTag">답변</strong> ${reply.repDate }
										</dt>
										<dd class="replyTxt">
											<i class="fa-solid fa-arrow-turn-up"></i> ${reply.repContent }
										</dd>
									</dl>
								</div>
							</c:if>
					...
					<div class="listBtmArea">
						<div class="btnListExDn"
							style="justify-content: left; float: left; display: flex;">
							<a href="<%=request.getContextPath()%>/member/qnaList"
						...
						<fieldset>
							...
							<div class="tbInquiryBx">
									...
									<div class="tbCell">
										<div class="mtcSltBx listLenSel">
											<input type="text" placeholder="제목을 입력하세요."
												value="${qna.qstnTitle }" name="qstnTitle" id="title"
												required />
										</div>
									</div>
								</div>
								<div class="tbRow">
									...
									<div class="tbCell">
										<textarea name="qstnContent" id="lb_inq_2" title="내용을 입력하세요."
											class="txArea" required>${qna.qstnContent }</textarea>
									</div>
								</div>
							</div>
